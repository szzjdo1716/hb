...
const I18N = {
  zh: {
    ...
    answer: "应看到",
    missing: "没有这张卡片。",
    pasteHint: "贴到 > 再按 Enter。",
    footer: "原创教学例子。不是任何书籍的复制。",
    lab: {
      title: "R 入门",
      lede: "安装到 >，再贴向量与数据框的短例子。练习在另一页。",
      searchPh: "R, 安装, NA, data.frame",
      empty: "没有匹配。试 R / 安装 / NA / data.frame",
      jump: "去练习",
      jumpHref: "r-practice.html",
    },
    practice: {
      title: "R 练习",
      lede: "每条先复制，贴到 >，再按 Enter。答案在详情里。",
      searchPh: "mean, data.frame, oak, 17",
      empty: "没有匹配。试 mean / data.frame / oak",
      jump: "回入门",
      jumpHref: "r.html",
    },
  },
  en: {
    ...
    pasteHint: "Paste at >, then press Enter.",
    footer: "Original teaching examples. Not a reproduction of any book.",
    lab: {
      title: "R essentials",
      lede: "Install, open >, then paste short vector and data-frame examples. Practice is a second page.",
      searchPh: "R, install, NA, data.frame",
      empty: "No match. Try R / install / NA / data.frame",
      jump: "Practice",
      jumpHref: "r-practice.html",
    },
    practice: {
      title: "R practice",
      lede: "Copy each drill, paste at >, press Enter. Answers are in Details.",
      searchPh: "mean, data.frame, oak, 17",
      empty: "No match. Try mean / data.frame / oak",
      jump: "Essentials",
      jumpHref: "r.html",
    },
  },
...
function officialLinks(card) {
  ...
  (card.extra_links || []).forEach((row) => {
    ...
    const local = String(row.href || "").indexOf("http") !== 0;
    const extra = local ? "" : ' target="_blank" rel="noopener noreferrer"';
    links.push(
      `<p><a href="${esc(row.href)}"${extra}>${esc(label)} — ${esc(row.href)}</a></p>`
    );
  });
...
function applyChrome() {
  ...
  if (footer) {
    footer.innerHTML =
      esc(ui.footer) +
      ' <a href="https://www.r-project.org" target="_blank" rel="noopener noreferrer">r-project.org</a> · ' +
      '<a href="https://cran.r-project.org" target="_blank" rel="noopener noreferrer">cran.r-project.org</a>';
  }
