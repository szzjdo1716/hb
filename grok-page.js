...
const I18N = {
  zh: {
    ...
    searchLabel: "搜索",
    searchPh: "build, 安装, 套餐, 技巧",
    catsLabel: "分类",
    all: "全部",
    status: (n, total) => `${n} / ${total} 张卡片`,
    empty: "没有匹配。试 build / 安装 / 套餐 / 技巧",
    exact: "精确匹配",
    copy: "复制",
    copied: "已复制",
    detail: "详情",
  ...
  en: {
    ...
    searchLabel: "Search",
    searchPh: "build, install, plans, tips",
    catsLabel: "Categories",
    all: "All",
    status: (n, total) => `${n} of ${total} cards`,
    empty: "No match. Try build / install / plans / tips",
    exact: "Exact match",
    copy: "Copy",
    copied: "Copied",
    detail: "Details",
  ...
  return cards.map((card) => {
    ...
    return {
      ...
      body: [
        ...
        (card.when_rows || [])
          .map((row) => [row.job_zh, row.job_en, row.pick_zh, row.pick_en].join(" "))
          .join(" "),
